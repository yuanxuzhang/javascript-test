...
3. 求模 余数 % 规则如下：  
    ...
    - 如果有一个操作数不是数值，则在后台调用 Number() 将其转换为数值，然后再应用上面的规则。  
### 加性操作 
加法和减法在ECMAScript中，这两个操作符有一系列的特殊行为，加法操作符也会在后台转换不同的数据类型。  
1. 加法 + 规则如下：   
    ...
    - **如果只有一个操作数是字符串，则将另一个操作数转换为字符串，然后再将这两个字符串拼接起来**。  
    - **如果有一个操作数是对象、数值或者布尔，则调用它们的 toString() 方法获取相应的字符串值，然后再应用前面关于字符串的规则**。
...
2. 减法 - 规则如下：  
    ...
    - 如果是 +0 减 -0，则结果是 -0。  
    - 如果是 -0 减 -0，则结果是 +0。  
    - 如果有一个操作数是字符串、布尔值、null或者undefined，则先在后台调用 Number() 函数将其转换为数值，然后再根据前面的规则执行减法计算。如果转换的结果是 NaN，则减法的结果就是 NaN。  
    - 如果有一个操作数是对象，则调用对象的 valueOf() 方法以取得表示该对象的数值。 如果得到的值时 NaN，则减法的结果就是 NaN。如果对象没有 valueOf() 方法，则调用 toString() 方法并将得到的字符串转换为数值。
### 关系操作符[Code]()
小于 <、大于 >、小于等于 <= 和大于等于 >= 用于对两个值进行比较，返回一个布尔值。关系操作符的操作数使用非数值时，也要进行数据转换或完成某些奇怪的操作。规则为：  
- 如果两个操作数都是数值，则执行数值比较。  
- **如果两个操作数都是字符串，则比较两个字符串对应的**`字符编码值`。 
    - `'A' < 'b'`在比较字符串时，实际比较的是两个字符串中**对应位置**的每个字符串的字符编码值，经过一番比较之后，再返回一个布尔值，大写字母的字符编码全部小于小写字母的字符编码值。需要先统一字符串的大小写。  
    - '23' < '3' '2'编码为 50，'3'字符编码值为 51。  
- **如果一个操作数是数值，则将另一个操作数转换为一个数值，然后进行数值比较**。  
- 如果一个操作数是对象，则调用这个对象得 valueOf() 方法，用到的结果按照前面的规则执行比较。如果对象没有 valueOf() 方法，则调用 toString()方法，并用得到的结果根据前面的规则执行比较。  
- 如果一个操作数是布尔值，则先将其转换为数值，然后再执行比较。  
- **任何操作数与 NaN 进行关系比较，结果都是 false**。  
### 相等操作符[Code](https://github.com/yuanxuzhang/javascript-test/blob/master/basic-concept/code/EqualOperator.js)  
   涉及到字符串、数值、布尔值和对象之间的比较，最早的 ECMAScript 中的相等 == 和不等操 != 作符会在执行比较之前，先将对象转换成相似的类型。之后引入了全等 === 和不全等 !== 操作符。  
1. 相等和不相等  == != 这两个操作符会先转换操作数（强制类型转换），然后再比较它们的相等性。规则为：  
    - 如果有一个操作数是布尔值，则在比较相等性之前先将其转换为数值（false--0 true--1）。  
    - 如果一个操作数是字符串，另一个操作数是数值，在比较相等性之前先将字符串转换为数值。  
    - 如果一个操作数是对象，另一个操作数不是，则调用对象的 valueOf() 方法，用到的基本类型值按照前面的规则进行比较。  
    - null 和 undefined 是相等的。  
    - **要比较相等性之前，不能将 null 和 undefined 转换为其他任何值**。  
    - 如果有一个操作符是 NaN，则相等操作符返回 false，而不相等操作符返回 true。 **即使两个操作符都是 NaN，相等操作符也返回false；因为按照规则，NaN 不等于 NaN**。  
    - 如果两个操作数都是对象，则比较他们是不是同一个对象。如果两个操作数都指向同一个对象，则相等性操作符返回 true；否则，返回 false。  
2. 全等和不全等  同相等和不相等区别**在比较之前不能转换操作数**。  
    - null == undefined 为真，因为它们是**类似的值**； null === undefined 为假，因为它们是**不同类型的值**。  
PS：由于相等性和不相等操作符存在类型转换问题，而为了保持代码中数据类型的完整性，推荐使用全等和不全等操作符。  
### 条件操作符  
`var max = (num1 > num2) ? num1 ：num2;`  
本质上，上面的代码含义为基于对 num1 > num2 求值结果，决定变量 max 赋什么值。如果为 true，则变量 max 赋 num1；如果求职结果为 false，则给变量 max 赋 num2 值。  
### 赋值操作符  
= 双操作数  右侧的值赋给左侧的值，如果在等于号钱再添加**乘性操作符**、**加性操作符**或**位操作符**，就可以完成复合赋值操作。  
`*= /= %=      += -=      <<=  >>= >>>=`  
上边操作符的主要目的是为了简化赋值操作，使用它们不会带来任何性能提升。  
### 逗号操作符  
1. 可以在一条语句中执行多个操作。`var num1 = 1, num2 = 2, num3 = 3;`。  
2. 声明多个变量。 `var num1,num2,num3;` 。   
3. 用于赋值，总是返回表达式中的最后一项。 `var num = (5, 1, 2, 8, 0)`。  
